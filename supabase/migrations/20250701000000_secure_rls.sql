/*
  # Reinstate Secure RLS Policies

  1. Hardening
    - Reapply strict row level security on operational tables
    - Policies depend on user role and branch membership
  2. Role logic
    - Admins (users.role = 'admin') can access everything
    - Branch users may access rows where their branch_id matches
    - Bookings are visible if from_branch_id or to_branch_id matches
*/
    ...
    OR EXISTS (
      ...
      WHERE u.id = auth.uid()
        AND (
          ...
          u.branch_id = public.bookings.from_branch_id OR
          u.branch_id = public.bookings.to_branch_id
        )
    )
  ) WITH CHECK (
    EXISTS (
  ...
  FOR ALL USING (
    ...
    OR EXISTS (
      ...
      WHERE u.id = auth.uid() AND (
        ...
        u.branch_id = b.from_branch_id OR
        u.branch_id = b.to_branch_id
      )
    )
  ) WITH CHECK (
    EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
