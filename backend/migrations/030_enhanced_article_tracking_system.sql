-- Enhanced Article Tracking System Migration
-- This migration adds comprehensive tracking capabilities including GPS, scanning, and analytics
    ...
    LOOP
        ...
        LOOP
            ...
            SELECT 
                ...
                -- Time calculations (placeholder - would need more complex logic)
                AVG(EXTRACT(EPOCH FROM (ash.scan_time - LAG(ash.scan_time) OVER (PARTITION BY ash.booking_id ORDER BY ash.scan_time)))/60)::INTEGER as avg_dwell_time,
...
-- Insert sample barcode patterns for existing bookings
DO $$
DECLARE
    booking_rec RECORD;
    barcode_prefix VARCHAR(10);
BEGIN
    -- Generate barcodes for existing bookings
    FOR booking_rec IN 
        SELECT b.id, b.lr_number, b.branch_id, b.organization_id
        FROM bookings b 
        WHERE b.created_at >= CURRENT_DATE - INTERVAL '30 days'
        LIMIT 100
    LOOP
        barcode_prefix := 'DC' || LPAD((booking_rec.id::text)::bigint % 1000000, 6, '0');
        
        INSERT INTO article_tracking_enhanced (
            booking_id,
            barcode,
